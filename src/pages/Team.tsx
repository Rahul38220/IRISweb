...
const STUDENTS = [
  {
    ...
    photo: 'https://i.ibb.co/yBmgB1nz/IMG-20260719-191839-1.jpg',
  ...
  },
  {
    ...
    photo: 'https://i.ibb.co/xS0ZbMH0/unnamed.jpg',
...
export default function Team() {
  ...
  return (
    <div style={{ background: 'var(--bg)', minHeight: '100vh', paddingTop: 64, fontFamily: 'Outfit,sans-serif' }}>

      {/* ── Page hero ─────────────────────────────────────────────────────── */}
      <section style={{ padding: '80px 24px 72px', borderBottom: '1px solid rgba(201,168,76,0.08)', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', inset: 0, backgroundImage: 'url(https://images.unsplash.com/photo-1759956445479-c436a50d9e59?w=1200&h=500&fit=crop&auto=format)', backgroundSize: 'cover', backgroundPosition: 'center', opacity: 0.05 }} />
        <div style={{ position: 'absolute', top: '50%', left: '30%', width: 600, height: 600, borderRadius: '50%', background: 'radial-gradient(circle,rgba(201,168,76,0.05),transparent 70%)', pointerEvents: 'none' }} />
      ...
      {/* ── Student cards with photos — LIGHT CREAM ───────────────────────── */}
      <section className="section-card-stack light-surface" style={{ padding: '100px 24px', background: '#f8f1e2', zIndex: 2 }}>
                ...
                <GlassCard accentColor={s.color} style={{ padding: 36, height: '100%' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'flex-start', gap: 20, marginBottom: 28 }}>
                    ...
                    <div style={{ flexShrink: 0, position: 'relative' }}>
                      <div style={{ width: 80, height: 80, borderRadius: 14, overflow: 'hidden', border: `2px solid ${s.color === 'var(--accent)' ? 'rgba(201,168,76,0.35)' : 'rgba(232,205,106,0.35)'}`, background: 'var(--surface)' }}>
                        <img
                          src={s.photo}
                          alt={s.name}
                          style={{ width: '100%', height: '100%', objectFit: 'cover', filter: 'grayscale(30%) brightness(0.9)' }}
                        />
                      </div>
                      {/* Gold dot overlay */}
                      <div style={{ position: 'absolute', bottom: 4, right: 4, width: 12, height: 12, borderRadius: '50%', background: s.color === 'var(--accent)' ? 'var(--accent)' : 'var(--accent-2)', border: '2px solid var(--card-inner)', boxShadow: '0 0 6px rgba(201,168,76,0.5)' }} />
                    </div>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 20, fontWeight: 800, color: 'var(--text)', letterSpacing: '-0.02em', marginBottom: 3 }}>{s.name}</div>
                      <div style={{ fontSize: 13, color: 'var(--text-2)', marginBottom: 10 }}>{s.role}</div>
                      <div style={{ display: 'inline-block', padding: '3px 12px', borderRadius: 999, fontSize: 10, fontFamily: 'JetBrains Mono,monospace', border: `1px solid ${s.color === 'var(--accent)' ? 'rgba(201,168,76,0.25)' : 'rgba(232,205,106,0.25)'}`, color: s.color, background: s.color === 'var(--accent)' ? 'rgba(201,168,76,0.08)' : 'rgba(232,205,106,0.08)' }}>
                        {s.focus}
                      </div>
                    </div>
                  </div>
                  <blockquote style={{ fontSize: 14, color: 'var(--text-2)', lineHeight: 1.76, fontStyle: 'italic', borderLeft: `2px solid rgba(201,168,76,0.3)`, paddingLeft: 16, marginBottom: 28 }}>
                    {s.quote}
                  </blockquote>
                  <div>
                    <div style={{ fontSize: 11, fontFamily: 'JetBrains Mono,monospace', color: 'var(--accent)', letterSpacing: '0.15em', textTransform: 'uppercase', marginBottom: 12 }}>Key Contributions</div>
                    <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: 10 }}>
                      {s.contributions.map((c, ci) => (
                        <li key={ci} style={{ display: 'flex', alignItems: 'flex-start', gap: 10, fontSize: 13, color: 'var(--text-2)', lineHeight: 1.6 }}>
                          <span style={{ width: 4, height: 4, borderRadius: '50%', background: 'var(--accent)', flexShrink: 0, marginTop: 7, opacity: 0.7 }} />
                          {c}
                        </li>
                      ))}
                    </ul>
      ...
      {/* ── Behind the project — full-width image strip ────────────────────── */}
      <section className="section-card-stack" style={{ position: 'relative', height: 280, overflow: 'hidden', zIndex: 3 }}>
        <img
          src="https://images.unsplash.com/photo-1489875347897-49f64b51c1f8?w=1600&h=560&fit=crop&auto=format"
          alt="Developers at work — code on screens"
          style={{ width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'center 30%', filter: 'grayscale(40%) brightness(0.45)' }}
        />
        {/* Gold gradient overlays */}
        <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(90deg,rgba(7,6,4,0.9) 0%,transparent 40%,transparent 60%,rgba(7,6,4,0.9) 100%)' }} />
        <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg,rgba(7,6,4,0.3) 0%,transparent 40%,rgba(7,6,4,0.7) 100%)' }} />
        {/* Centered label */}
        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 10, textAlign: 'center' }}>
          <div style={{ fontFamily: 'JetBrains Mono,monospace', fontSize: 10, letterSpacing: '0.3em', color: 'var(--accent)', textTransform: 'uppercase' }}>Late nights &amp; early mornings</div>
          <div style={{ fontSize: 'clamp(20px,3vw,30px)', fontWeight: 900, color: 'var(--text)', letterSpacing: '-0.02em', maxWidth: 500 }}>Built in a school lab.<br />Powered by curiosity.</div>
        </div>
      </section>

      {/* ── Project journey ────────────────────────────────────────────────── */}
      <section className="section-card-stack" style={{ padding: '100px 24px 100px', background: 'var(--surface)', zIndex: 4 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(280px,1fr))', gap: 14 }}>
            {JOURNEY.map((j, i) => (
              <div key={i} className="reveal" style={{ animationDelay: `${i * 80}ms` }}>
                <GlassCard style={{ padding: '26px 22px', height: '100%' }}>
                  <div style={{ fontSize: 24, marginBottom: 12 }}>{j.icon}</div>
      ...
      {/* ── Mentor cards — LIGHT CREAM ──────────────────────────────────────── */}
      <section className="section-card-stack light-surface" style={{ padding: '100px 24px', background: '#f8f1e2', zIndex: 5 }}>
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 60 }}>
              ...
              <div key={i} className="reveal" style={{ animationDelay: `${i * 100}ms` }}>
                <GlassCard accentColor={m.color} style={{ padding: '32px 28px', height: '100%' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 20 }}>
                    <div style={{ width: 50, height: 50, borderRadius: '50%', background: 'rgba(201,168,76,0.1)', border: '1px solid rgba(201,168,76,0.25)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--accent)', fontWeight: 800, fontSize: 15, flexShrink: 0, fontFamily: 'JetBrains Mono,monospace' }}>{m.initials}</div>
                    <div>
                  ...
                  <span style={{ display: 'inline-block', padding: '4px 12px', borderRadius: 999, fontSize: 10, fontFamily: 'JetBrains Mono,monospace', border: '1px solid rgba(201,168,76,0.2)', color: 'var(--accent)', background: 'rgba(201,168,76,0.06)' }}>{m.badge}</span>
                </GlassCard>
              </div>
            ))}
          </div>
      ...
      {/* ── Acknowledgements — no section-card: consecutive light section ──── */}
      <section style={{ padding: '0 24px 100px', background: '#f8f1e2' }}>
        <div style={{ maxWidth: 800, margin: '0 auto' }}>
          <div className="reveal" style={{ padding: '48px 40px', borderRadius: 22, border: '1px solid rgba(201,168,76,0.15)', background: 'rgba(201,168,76,0.03)', backdropFilter: 'blur(20px)', position: 'relative', overflow: 'hidden' }}>
            ...
            <div style={{ fontSize: 15, color: 'var(--text-2)', lineHeight: 1.8, display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <p style={{ fontFamily: 'JetBrains Mono,monospace', fontSize: 12, color: 'rgba(122,110,80,0.5)', marginTop: 8 }}>
                — Pratham Joshi & Hariom Bhimani
              </p>
            </div>
            <div style={{ position: 'absolute', bottom: 0, left: '10%', right: '10%', height: 1, background: 'linear-gradient(90deg,transparent,rgba(201,168,76,0.4),transparent)' }} />
          </div>
        </div>
      </section>

      {/* ── CTA ─────────────────────────────────────────────────────────────── */}
      <section className="section-card-stack" style={{ padding: '0 24px 100px', textAlign: 'center', background: 'var(--bg)', zIndex: 6 }}>

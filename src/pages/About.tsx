...
function CameraHUD() {
  ...
  return (
    <div style={{ borderRadius: 16, overflow: 'hidden', border: '1px solid rgba(255,255,255,0.09)', background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(20px)' }}>
      ...
      <div style={{ position: 'relative', aspectRatio: '16/9', background: 'linear-gradient(135deg,#0a0800,#050400)', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', inset: 0, opacity: 0.06, backgroundImage: 'linear-gradient(rgba(201,168,76,0.8) 1px,transparent 1px),linear-gradient(90deg,rgba(201,168,76,0.8) 1px,transparent 1px)', backgroundSize: '40px 40px' }} />
        {/* HUD top */}
        <div style={{ position: 'absolute', top: 10, left: 12, right: 12, display: 'flex', justifyContent: 'space-between', ...mono, fontSize: 9, color: 'rgba(201,168,76,0.6)' }}>
        ...
        <div style={{ position: 'absolute', top: '20%', left: '56%', width: '22%', height: '40%' }}>
          <div style={{ position: 'absolute', inset: 0, border: '1px solid rgba(201,168,76,0.45)', borderRadius: 2 }} />
          <div style={{ position: 'absolute', bottom: -18, left: 0, right: 0, textAlign: 'center' }}>
...
export default function About() {
  ...
  return (
    <div style={{ background: 'var(--bg)', minHeight: '100vh', paddingTop: 64 }}>
      ...
      {/* ── Page hero ─────────────────────────────────────────────────────── */}
      <section className="section-card" style={{ padding: '80px 24px 80px', borderBottom: '1px solid rgba(201,168,76,0.08)', position: 'relative', overflow: 'hidden', background: 'var(--bg)', zIndex: 2 }}>
        {/* Background image */}
        <div style={{ position: 'absolute', inset: 0, backgroundImage: 'url(https://images.unsplash.com/photo-1741356474365-5f0041f89eaa?w=1600&h=700&fit=crop&auto=format)', backgroundSize: 'cover', backgroundPosition: 'center 40%', opacity: 0.05 }} />
        <div style={{ position: 'absolute', top: 0, left: '40%', width: 600, height: 600, borderRadius: '50%', background: 'radial-gradient(circle,rgba(201,168,76,0.06),transparent 70%)', pointerEvents: 'none' }} />
      ...
      {/* ── Problem & Solution — LIGHT CREAM ──────────────────────────────── */}
      <section className="section-card light-surface" style={{ padding: '100px 24px', background: '#f8f1e2', zIndex: 3 }}>
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          ...
          <div className="reveal" style={{ marginTop: 48, padding: '32px 36px', borderRadius: 18, background: 'rgba(201,168,76,0.1)', border: '1px solid rgba(160,120,48,0.3)', textAlign: 'center' }}>
      ...
      <section className="section-card" style={{ padding: '80px 24px 100px', background: 'var(--surface)', zIndex: 4 }}>
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 64 }}>
      ...
      {/* ── Tech deep-dives ────────────────────────────────────────────────── */}
      <section className="section-card" style={{ padding: '80px 24px 100px', background: 'var(--bg)', zIndex: 5 }}>
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
            {TECH_DEEP.map((t, i) => (
              <div key={i} className="reveal" style={{ animationDelay: `${i * 80}ms` }}>
                {/* Grid goes INSIDE the card content, not on the card itself */}
                <GlassCard accentColor={t.color}>
                  <div style={{ display: 'grid', gridTemplateColumns: 'minmax(140px,190px) 1fr', gap: 28, alignItems: 'start' }}>
                    <div>
                      <div style={{ fontSize: 11, fontFamily: 'JetBrains Mono,monospace', color: t.color, letterSpacing: '0.15em', textTransform: 'uppercase', marginBottom: 8 }}>{t.name}</div>
                      <div style={{ fontSize: 12, color: 'var(--text-2)', lineHeight: 1.55 }}>{t.full}</div>
                    </div>
      ...
      <section className="section-card" style={{ padding: '80px 24px 100px', background: 'var(--bg)', zIndex: 6 }}>
        <div style={{ maxWidth: 860, margin: '0 auto' }}>
          ...
          <div className="reveal" style={{ background: 'rgba(201,168,76,0.04)', border: '1px solid rgba(201,168,76,0.12)', borderRadius: 18, padding: '32px 24px' }}>
            <ResponsiveContainer width="100%" height={260}>
              <BarChart
                data={[
                  { condition: 'Full light', accuracy: 94 },
                  { condition: '75% light', accuracy: 88 },
                  { condition: '50% light', accuracy: 60 },
                  { condition: '25% light', accuracy: 32 },
                ]}
                margin={{ top: 4, right: 16, bottom: 0, left: -10 }}
              >
                <XAxis dataKey="condition" tick={{ fill: '#7a6e50', fontSize: 11, fontFamily: 'JetBrains Mono,monospace' }} axisLine={false} tickLine={false} />
                <YAxis tick={{ fill: '#7a6e50', fontSize: 10, fontFamily: 'JetBrains Mono,monospace' }} axisLine={false} tickLine={false} unit="%" domain={[0, 100]} />
                <Tooltip
                ...
                <Bar dataKey="accuracy" radius={[4, 4, 0, 0]}>
                  {[{ color: '#4a9e6a' }, { color: '#c9a84c' }, { color: '#d4a43a' }, { color: '#b85040' }].map((e, i) => (
      ...
      {/* ── Bottom CTA ─────────────────────────────────────────────────────── */}
      <section className="section-card" style={{ padding: '80px 24px 100px', textAlign: 'center', background: 'var(--bg)', zIndex: 7 }}>

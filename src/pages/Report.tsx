...
const RESULTS = [
  ...
  { metric: 'Frame Latency (all systems)', value: '<50ms', detail: 'On Intel Core i5 laptop, no GPU, 720p input', color: 'var(--warn)' },
...
export default function Report() {
  ...
  return (
    <div style={{ background: 'var(--bg)', minHeight: '100vh', paddingTop: 64 }}>
      ...
      {/* ── Download card ──────────────────────────────────────────────────── */}
      <section className="section-card" style={{ padding: '80px 24px', background: 'var(--bg)', zIndex: 2 }}>
        <div style={{ maxWidth: 900, margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(280px,1fr))', gap: 40, alignItems: 'center' }}>
          ...
          <div className="reveal">
            <div style={{ borderRadius: 18, overflow: 'hidden', border: '1px solid rgba(255,255,255,0.09)', background: 'rgba(255,255,255,0.025)', backdropFilter: 'blur(20px)', aspectRatio: '3/4', display: 'flex', flexDirection: 'column', position: 'relative', maxHeight: 420 }}>
              {/* Header band */}
              <div style={{ padding: '14px 18px', background: 'rgba(201,168,76,0.08)', borderBottom: '1px solid rgba(201,168,76,0.15)', display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <div style={{ flexGrow: 1, padding: '24px 20px', display: 'flex', flexDirection: 'column', gap: 12 }}>
                ...
                {[90,65,80,55,70,45,80,60,50].map((w,i) => (
                  <div key={i} style={{ height: i===0?5:3.5, borderRadius: 999, background: i===0?'rgba(201,168,76,0.35)':'rgba(201,168,76,0.07)', width: `${w}%` }} />
                ))}
          ...
          <div className="reveal" style={{ animationDelay: '160ms' }}>
            ...
            <a href="https://drive.google.com/file/d/1haw2Lm85udbwSATozTcT20EWEhRznu3_/view?usp=sharing" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, width: '100%', padding: '15px 0', borderRadius: 12, color: '#070604', fontWeight: 700, fontSize: 14, textDecoration: 'none', background: 'linear-gradient(135deg,var(--accent),var(--accent-2))', boxShadow: '0 0 28px rgba(201,168,76,0.38)', transition: 'transform 0.2s,box-shadow 0.2s' }}
               onMouseEnter={e => { (e.currentTarget as HTMLElement).style.transform = 'scale(1.025)'; (e.currentTarget as HTMLElement).style.boxShadow = '0 0 44px rgba(201,168,76,0.6)' }}
               onMouseLeave={e => { (e.currentTarget as HTMLElement).style.transform = ''; (e.currentTarget as HTMLElement).style.boxShadow = '0 0 28px rgba(201,168,76,0.38)' }}>
              📄 Download PDF Report
            </a>
      ...
      {/* ── Table of contents ──────────────────────────────────────────────── */}
      <section className="section-card" style={{ padding: '80px 24px 100px', background: 'var(--surface)', zIndex: 3 }}>
      ...
      {/* ── Key results — LIGHT CREAM ───────────────────────────────────────── */}
      <section className="section-card light-surface" style={{ padding: '80px 24px 100px', background: '#f8f1e2', zIndex: 4 }}>
            ...
            {RESULTS.map((r, i) => (
              <div key={i} className="reveal" style={{ animationDelay: `${i * 70}ms` }}>
                <GlassCard accentColor={r.color} style={{ padding: '26px 24px' }}>
                  ...
                  <div style={{ fontSize: 12, color: 'var(--text-2)', lineHeight: 1.55 }}>{r.detail}</div>
                </GlassCard>
              </div>
            ))}
          </div>
      ...
      {/* ── CTA ────────────────────────────────────────────────────────────── */}
      <section className="section-card" style={{ padding: '80px 24px 100px', textAlign: 'center', background: 'var(--bg)', zIndex: 5 }}>
